...
void c_kv3_parser::parse(const std::string &content)
{
...
std::string c_kv3_parser::get_value(const std::string &path) const
{
...
std::vector<std::string> c_kv3_parser::find_key_paths_with_key_name(const std::string &search_key)
{
...
void c_kv3_parser::find_key_paths_with_key_name(const value_t &root, const std::string &search_key, std::vector<std::string> &paths, const std::string &current_path) const
{
...
void c_kv3_parser::skip_comments_and_metadata()
{
...
void c_kv3_parser::skip_whitespace()
{
...
void c_kv3_parser::skip_comments()
{
    while (index < content.size() && content[index] == '/')
    {
        index = content.find('\n', index) + 1;
...
size_t c_kv3_parser::get_key_or_value_end()
{
...
value_struct_t c_kv3_parser::parse_value()
{
    skip_comments();
    skip_whitespace();
...
value_struct_t c_kv3_parser::parse_byte_array()
{
    skip_whitespace();
    ++index;
...
value_struct_t c_kv3_parser::parse_object()
{
    object_t obj;
    skip_whitespace();
    ++index;
...
value_struct_t c_kv3_parser::parse_array()
{
    array_t arr;
    skip_whitespace();
    ++index;
...
std::string c_kv3_parser::join(const std::vector<std::string> &vec, const std::string &delim)
{
