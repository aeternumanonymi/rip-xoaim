...
private:
    ...
    struct HashFixedDataInternal_t
    {
        ...
        {
            tData data;
            const auto address = reinterpret_cast<uintptr_t>(this) + 0x10;
            if (!nt::success(g_process.read_memory(address, &data, sizeof(tData))))
            {
                return {};
            }
        ...
        {
            uintptr_t next_entry_address = 0;
            if (!nt::success(g_process.read_memory(reinterpret_cast<uintptr_t>(this) + 0x20, &next_entry_address, sizeof(uintptr_t))))
            {
                return nullptr;
            }

            return reinterpret_cast<HashFixedDataInternal_t<tData> *>(next_entry_address);
        }
    };
